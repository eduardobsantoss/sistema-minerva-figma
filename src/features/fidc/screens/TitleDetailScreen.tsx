...
const statusTone: Record<TitleStatus, { bg: string; fg: string; iconBg: string }> = {
  CONFIRMADO: {
    ...
    fg: 'var(--success-dark)',
    iconBg: 'var(--success-base)',
  },
  PENDENTE: {
    bg: 'var(--warning-light)',
    fg: 'var(--warning-dark)',
    iconBg: 'var(--warning-base)',
  },
  VENCIDO: {
    bg: 'var(--danger-light)',
    fg: 'var(--danger-dark)',
    iconBg: 'var(--danger-base)',
  },
};
    ...
    <div className="flex flex-col" style={{ gap: 24 }}>
      ...
      >
        ...
        <div style={{ flex: 1, position: 'relative', zIndex: 1 }}>
          ...
          <div
            style={{ fontSize: 'var(--text-xs)', color: 'rgba(255,255,255,0.65)', marginTop: 8 }}
          >
...
function DetailsTab({ title, klass }: { title: Title; klass: FidcClass }) {
  return (
    <div className="flex flex-col" style={{ gap: 32 }}>
      <Section title="Informações do Título">
        <div className="grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', gap: 24 }}>
          <Field label="Classe de Origem" value={klass.name} />
          <Field label="ID de Lastro" value={`LST-${title.numero}`} />
          <Field label="Número" value={<span className="flex items-center" style={{ gap: 6 }}>#{title.numero}<CopyButton value={title.numero} /></span>} />
          <Field label="Tipo de Ativo" value={title.lastro.replace('_', '-')} />
        </div>
      </Section>

      <Section title="Datas e Prazos">
        <div className="grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', gap: 24 }}>
          <Field label="Data de Emissão" value={title.emissao} />
          <Field label="Data de Vencimento" value={title.vencimento} />
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div className="flex items-center" style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginTop: 2, gap: 4 }}>
...
function AnexosTab({ title }: { title: Title }) {
  const files = [
    { name: `NFE-${title.numero}.xml`, size: '14 KB', type: 'XML' },
    { name: `DANFE-${title.numero}.pdf`, size: '128 KB', type: 'PDF' },
    { name: `Canhoto-${title.numero}.jpg`, size: '342 KB', type: 'IMG' },
  ];
  return (
...
function MovimentoTab({ title }: { title: Title }) {
  const events = [
    { date: title.emissao, label: 'Título emitido pelo cedente', tone: 'info' },
    { date: title.emissao, label: 'Registro enviado à registradora (CERC)', tone: 'info' },
    {
      date: title.emissao,
      label: 'Confirmação física do sacado',
      tone: title.status === 'CONFIRMADO' ? 'success' : 'warning',
    },
    {
      date: title.vencimento,
      label:
        title.status === 'VENCIDO'
          ? 'Inadimplência identificada'
          : 'Vencimento programado',
      tone: title.status === 'VENCIDO' ? 'danger' : 'neutral',
    },
  ];
      ...
      <div className="flex flex-col" style={{ gap: 0, position: 'relative' }}>
        {events.map((e, i) => (
          <div
            ...
            style={{ gap: 16, padding: '12px 0', position: 'relative' }}
          >
...
function AccrualTab({ title }: { title: Title }) {
  const rows = [
    { data: title.emissao,   taxa: '1,8500%', base: '252', accrual: brl(title.vrNominal * 0.0185 / 252), acumulado: brl(title.vrNominal * 0.0185 / 252 * 3) },
    { data: title.emissao,   taxa: '1,8500%', base: '252', accrual: brl(title.vrNominal * 0.0185 / 252), acumulado: brl(title.vrNominal * 0.0185 / 252 * 2) },
    { data: title.vencimento, taxa: '1,8500%', base: '252', accrual: brl(title.vrNominal * 0.0185 / 252), acumulado: brl(title.vrNominal * 0.0185 / 252) },
  ];
  return (
...
function ConfirmacoesTab({ title }: { title: Title }) {
  const confirmacoes = [
    { data: title.emissao, tipo: 'Confirmação do Sacado', resultado: title.status === 'CONFIRMADO' ? 'CONFIRMADO' : 'PENDENTE', obs: 'Via portal eletrônico' },
    { data: title.emissao, tipo: 'Validação de Lastro', resultado: 'CONFIRMADO', obs: 'Documento verificado' },
    { data: title.emissao, tipo: 'Registro Registradora', resultado: 'CONFIRMADO', obs: 'B3 — protocolo #82341' },
  ];
...
function MovimentacoesTab({ title }: { title: Title }) {
  const [sub, setSub] = useState<MovSub>('registro');
  const registros = [
    { data: title.emissao,    operacao: 'Inclusão',  registradora: 'B3',  protocolo: '#82341', valor: brl(title.vrNominal) },
    { data: title.vencimento, operacao: 'Atualização', registradora: 'B3', protocolo: '#82411', valor: brl(title.vrNominal * 0.98) },
  ];
  const cessoes = [
    { data: title.emissao, cedente: title.cedente, cessionario: 'FIDC Fundo', valor: brl(title.vrNominal), status: 'LIQUIDADO' },
  ];
  return (
            ...
            {cessoes.map((r, i) => (
              <div key={i} className="grid items-center" style={{ gridTemplateColumns: '1fr 1.4fr 1.4fr 1fr 1fr', padding: '14px 16px', borderTop: '1px solid var(--border-default)', fontSize: 'var(--text-sm)' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontSize: 9, fontWeight: 'var(--weight-bold)', letterSpacing: '0.10em', padding: '4px 8px', borderRadius: '9999px', background: 'var(--success-light)', color: 'var(--success-dark)' }}>{r.status}</span>
                </div>
              </div>
            ))}
          </div>
