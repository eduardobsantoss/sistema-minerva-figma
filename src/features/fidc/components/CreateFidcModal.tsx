...
interface Props {
  onClose: () => void;
  onCreate: (data: NewFidcData) => void;
}
...
export function CreateFidcModal({ onClose, onCreate }: Props) {
  const [stepIdx, setStepIdx] = useState(0);
  const [form, setForm] = useState<NewFidcData>({
    cnpj: '',
    identificacao: '',
    razaoSocial: '',
    nomeFantasia: '',
  ...
  const set = (field: keyof NewFidcData) =>
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
      setForm((prev) => ({ ...prev, [field]: e.target.value }));

  const setVal = (field: keyof NewFidcData) => (v: string) =>
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 40 }}>
          ...
          {step.key === 'info' && (
            <StepGrid>
              {/* Row 1: CNPJ | Identificação | Tipo Empresa */}
              <FormField
                label="CNPJ do Veículo"
              ...
              <FormField
                ...
                placeholder="Ex: CERES AGRO I"
                span={5}
                value={form.identificacao}
                onChange={set('identificacao')}
              />
              <SelectField
                label="Tipo de Empresa"
                options={['Fundo', 'Companhia', 'Sociedade Limitada']}
                placeholder="Selecione"
                span={3}
                value={form.tipoEmpresa}
                onChange={setVal('tipoEmpresa')}
              />
              ...
              {/* Row 3: Data Constituição | Natureza Legal | Atividade Principal */}
              <FormField
                label="Data de Constituição"
                type="date"
                span={3}
                value={form.dataConstituicao}
                onChange={set('dataConstituicao')}
              />
              <SelectField
                label="Natureza Legal"
                options={['Condomínio Fechado', 'Condomínio Aberto', 'Sociedade Anônima']}
                placeholder="Selecione"
                span={3}
                value={form.naturezaLegal}
                onChange={setVal('naturezaLegal')}
              />
              <FormField
                label="Atividade Principal"
                placeholder="Direitos Creditórios"
                span={6}
                value={form.atividadePrincipal}
                onChange={set('atividadePrincipal')}
              />

              {/* Row 4: Tipo do Fundo | Categoria CVM */}
              <SelectField
                label="Tipo do Fundo"
                options={['ABERTO', 'FECHADO']}
                placeholder="Selecione"
                span={6}
                value={form.tipoFundo}
                onChange={setVal('tipoFundo')}
              />
              <SelectField
                label="Categoria CVM"
                options={['FIDC', 'FIDC-NP', 'FIDC Agro']}
                placeholder="Selecione"
                span={6}
                value={form.categoriaCvm}
                onChange={setVal('categoriaCvm')}
              />
            </StepGrid>
          )}
          ...
          {step.key === 'contato' && (
            <div className="flex flex-col" style={{ gap: 20 }}>
              <div>
                ...
                <StepGrid>
                  <FormField
                    label="E-mail do Veículo"
                    placeholder="contato@fidc.com.br"
                    type="email"
                    span={6}
                    value={form.email}
                    onChange={set('email')}
