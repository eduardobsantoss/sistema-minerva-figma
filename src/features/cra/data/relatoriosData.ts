...
export function mockPortfolioTitulos(funds: PortfolioFundRef[]): PortfolioTituloRow[] {
  return funds.flatMap((f, fi) =>
    Array.from({ length: 4 }, (_, i) => {
      ...
      const valorNum = 85_000 + fi * 40_000 + i * 27_500;
      return {
        id: `${f.id}-t${i + 1}`,
        fundo: f.nome,
        fundoId: f.id,
...
export function mockPortfolioSacados(funds: PortfolioFundRef[]): PortfolioSacadoRow[] {
  return funds.flatMap((f, fi) =>
    SACADOS.slice(0, 3).map((sacado, i) => ({
      ...
      fundoId: f.id,
      sacado,
      documento: `${String(10 + fi).padStart(2, '0')}.${String(100 + i * 11).padStart(3, '0')}.${String(200 + fi).padStart(3, '0')}/0001-${String(10 + i).padStart(2, '0')}`,
      limite: brl(1_200_000 + fi * 350_000 + i * 180_000),
    })),
  );
}
