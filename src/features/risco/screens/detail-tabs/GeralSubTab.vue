...
<template>
  <div class="flex flex-col" style="gap: 20px">
    <TabCard title="Confirmação de Cessões" :icon="ClipboardCheck">
      <div class="grid" style="grid-template-columns: repeat(12, 1fr); gap: 16px">
        ...
        <div style="grid-column: span 5">
          <PctInput label="* Confirmação p/ clientes novos (Pós-Desembolso)" :value="form.confirmacaoClientesNovosPct" @change="form.confirmacaoClientesNovosPct = $event" />
        </div>
        <div style="grid-column: span 5">
          <PctInput label="* Confirmação p/ clientes antigos (Pós-Desembolso)" :value="form.confirmacaoClientesAntigosPct" @change="form.confirmacaoClientesAntigosPct = $event" />
        </div>
      </div>
    </TabCard>
    ...
    <TabCard title="Sacado" :icon="Users">
      <div class="grid" style="grid-template-columns: repeat(12, 1fr); gap: 16px">
        <div style="grid-column: span 4">
          <PctInput label="* Notificação dos sacados" :value="form.notificacaoSacadosPct" @change="form.notificacaoSacadosPct = $event" />
        </div>
        <div style="grid-column: span 5">
          <PctInput label="* Concentr. máxima por sacado (limite)" :value="form.concentracaoMaximaSacadoPct" @change="form.concentracaoMaximaSacadoPct = $event" />
        </div>
      </div>
    </TabCard>
    ...
    <TabCard title="NF de Entrega Futura" :icon="Truck">
      <div class="grid" style="grid-template-columns: repeat(12, 1fr); gap: 16px">
        <div style="grid-column: span 4">
          <ToggleRow label="Pode operar" :on="form.nfEntregaFuturaPodeOperar" @toggle="form.nfEntregaFuturaPodeOperar = !form.nfEntregaFuturaPodeOperar" />
        </div>
        <div style="grid-column: span 5">
          <PctInput
            ...
            @change="form.nfEntregaFuturaOperacaoMaximaPct = $event"
          />
        </div>
      </div>
    </TabCard>
    ...
    <TabCard title="Crédito e Validade Serasa" :icon="ShieldCheck">
      <div class="flex flex-col" style="gap: 16px">
        <div class="grid" style="grid-template-columns: repeat(12, 1fr); gap: 16px">
          <div style="grid-column: span 5">
            <ToggleRow label="Pré-aprovação do sacado pelo crédito" :on="form.creditoPreAprovacaoSacado" @toggle="form.creditoPreAprovacaoSacado = !form.creditoPreAprovacaoSacado" />
          </div>
          <div style="grid-column: span 3">
        ...
        <div class="grid" style="grid-template-columns: repeat(12, 1fr); gap: 16px">
          ...
          <div style="grid-column: span 5">
            <ToggleRow label="Necessita aval do cônjuge" :on="form.necessitaAvalConjuge" @toggle="form.necessitaAvalConjuge = !form.necessitaAvalConjuge" />
          </div>
        </div>
      </div>
    </TabCard>
    ...
    <TabCard title="Exceções de Concentração por Sacado" :icon="UserCheck">
      <div class="flex flex-col" style="gap: 16px">
        ...
        <EmptyState v-if="form.excecoesConcentracao.length === 0" :icon="UserCheck" title="Nenhuma exceção cadastrada" hint='Use o formulário acima para cadastrar exceções de concentração por sacado.' />
        <div v-else style="border: 1px solid var(--border-default); border-radius: var(--radius-lg); overflow: hidden">
          <div class="grid items-center" style="grid-template-columns: 1.4fr 1fr 0.7fr 40px; padding: 10px 16px; background: var(--surface-sunken); font-size: 10px; font-weight: var(--weight-bold); letter-spacing: 0.08em; color: var(--text-muted); text-transform: uppercase">
            <div>Sacado</div><div>CPF/CNPJ</div><div style="text-align: right">Percentual</div><div />
          </div>
    ...
    <TabCard title="Avalistas com Obrigatoriedade de Assinatura" :icon="FileSignature" has-save @save="handleSave">
      <EmptyState v-if="form.avalistas.length === 0" :icon="FileSignature" title="Nenhum avalista cadastrado" hint="Avalistas obrigatórios de assinatura são herdados do cadastro de cedentes/avalistas do grupo." />
      <div v-else style="border: 1px solid var(--border-default); border-radius: var(--radius-lg); overflow: hidden">
        <div class="grid items-center" style="grid-template-columns: 1.4fr 1fr 130px 40px; padding: 10px 16px; background: var(--surface-sunken); font-size: 10px; font-weight: var(--weight-bold); letter-spacing: 0.08em; color: var(--text-muted); text-transform: uppercase">
          <div>Avalista</div><div>CPF/CNPJ</div><div>Assinatura obrigatória</div><div />
        </div>
        <div v-for="a in form.avalistas" :key="a.id" class="grid items-center" style="grid-template-columns: 1.4fr 1fr 130px 40px; padding: 10px 16px; border-top: 1px solid var(--border-default); font-size: var(--text-sm)">
          <div style="font-weight: var(--weight-semibold); color: var(--text-strong)">{{ a.nome }}</div>
          <div style="font-variant-numeric: tabular-nums; color: var(--text-muted)">{{ a.documento }}</div>
          <div>
            <input type="checkbox" :checked="a.obrigatorio" @change="toggleAvalistaObrigatorio(a.id)" />
          </div>
          <div class="flex justify-end">
